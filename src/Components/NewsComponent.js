...
export default class NewsComponent extends Component {
  ...
  captial = (str) => str.charAt(0).toUpperCase() + str.slice(1);
  ...
  componentDidMount() {
    ...
    this.observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && this.state.articles.length < 35) {
          this.setState(
  ...
  render() {
    ...
    return (
      <div className="newscomp">
        ...
        <h4 className="latest mx-3 px-3 py-3 align-self-end">
          Top <i className="news">Stories</i>
        </h4>
        <br />
        {this.state.loading && <Spinner />}
        {!this.state.loading && (
          <h5 className="cat">{this.captial(this.props.category)}</h5>
        )}
        ...
        <div ref={this.loader} style={{ height: "10px", marginBottom: "20px" }} />

        {!this.state.loading && (
          <div className="footer bg-dark">
            <center>
              <h4 className="bird py-3">
                <a className="footerhome" href="/">
                  <img className="im" src="bird-2-128.png" alt="" width={20} height={20} />{" "}
                  <i>News Bird</i>
                </a>
              </h4>
            </center>
            <div className="categories d-flex align-items-center justify-content-center">
              <h6>
                <Link className="li" to="/business">
                  Business
                </Link>
              </h6>
              <h6>
                <Link className="li" to="/science">
                  Science
                </Link>
              </h6>
              <h6>
              ...
              <h6>
                <Link className="li" to="/health">
                  Health
                </Link>
              </h6>
              <h6>
                <Link className="li" to="/tech">
                  Tech
                </Link>
              </h6>
              <h6>
